...
async function main() {
  ...
  for (const product of products) {
    if (product.blockchainProductId) {
      ...
      try {
        ...
        if (onChain && onChain[1] === product.productCode) {
          valid = true;
          console.log(`  ✓ Product ${product.productCode} properly matches on-chain ID ${product.blockchainProductId}`);
        } else {
          console.log(`  ⚠️ Product ${product.productCode} has ID ${product.blockchainProductId}, but on-chain that ID belongs to '${onChain ? onChain[1] : 'unknown'}'`);
        }
      } catch (e) {
        console.log(`  ⚠️ Product ${product.productCode} has ID ${product.blockchainProductId}, but it does not exist on-chain: ${e.message}`);
      }
  ...
  for (const product of products) {
    try {
      ...
      if (onChain && Number(onChain[0]) > 0) {
        const realId = onChain[0].toString();
  ...
  for (const tx of registeredEvents) {
    if (tx.entityId) {
      try {
        ...
        if (onChainProd && onChainProd[1]) {
          const matchingDbProd = await prisma.product.findUnique({
            where: { productCode: onChainProd[1] },
          });
